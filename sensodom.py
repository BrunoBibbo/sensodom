...
def localizacion(balizas, real, ideal, centro, radio, mostrar=0):
  ...
  # sensorial, dentro de una región concreta.
  mejor_pose = []
  mejor_peso = -1
  ...
  for i in range(2*r):
    for j in range(2*r):
      ...
      if (((centro[0]-x)*(centro[0]-x)) + ((centro[1]-y)*(centro[1]-y))) <= (radio*radio):
...
pioneer = robot()
pioneer.set_noise(0,0,.1)   # Ruido lineal / radial / de sensado
pioneer.set(*P_INICIAL)     # operador 'splat'
...
random.seed(0)
localizacion(objetivos,real,pioneer,[2,2],3,mostrar=1) #Para que funcione bien ponerlo a cero o eliminarlo
trayectoria = [pioneer.pose()]  # Trayectoria percibida
trayectreal = [real.pose()]     # Trayectoria seguida
print pioneer.pose()
print real.pose()
...
for punto in objetivos:
  while distancia(trayectoria[-1],punto) > EPSILON and len(trayectoria) <= 1000:
    ...
    if HOLONOMICO:
      ...
      pioneer.move_triciclo(w,v,LONGITUD)
      real.move_triciclo(w,v,LONGITUD)
    trayectoria.append(pioneer.pose())
